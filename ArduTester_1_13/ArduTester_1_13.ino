...
int melody_error[] = {392, 262, 220, 196};
int durations_error[] = {8, 8, 8, 4};
...
#define BAUD_RATE 9600
  ...
  if (tmp)
  {
    ...
    wait_about3s();
    BUZZER_TESTING();
    // time to read the Timeout message
    switch_tester_off();
    return 0;
  }
...
#ifdef WITH_UJT
  // check for UJT
  if (PartFound == PART_DIODE && NumOfDiodes == 2 // UJT is detected as 2 diodes E-B1 and E-B2...
...
#warning Capacity measurement from high to low not possible for diodes without PULLUP_DISABLE option!
  ...
  else if (PartFound == PART_TRANSISTOR)
  {
    ...
    if (vak_diode_nr < 5)
    {
      ...
      an_cat = (((PartMode == PART_MODE_NPN) && (ntrans.c > ntrans.e)) || ((PartMode != PART_MODE_NPN) && (ptrans.c < ptrans.e)));
#endif
      if (an_cat)
      {
        lcd_MEM_string(AnKat_str); //"->|-"
...
#define LINE_BELOW_ICON ((ICON_HEIGHT / 8) / ((FONT_HEIGHT + 7) / 8))
      ...
      if (_trans->ice0 < 4800)
      { // can't trust cutoff voltage if close to 5V supply voltage, since then the transistor may not have been cut off at all
...
resistor_out:
  if (ResistorsFound != 0)
  {
    ...
    { // multiple resistors found, R-Max suchen
